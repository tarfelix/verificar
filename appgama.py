...
# Usar uma classe para as chaves evita erros de digitação e centraliza a gestão.
class SK:
    LOGGED_IN = "logged_in"
    USERNAME = "username"
...
st.markdown("""
<style>
    /* Estilo para caixas de texto com highlight */
    ...
    /* Alinhamento vertical para o título */
...
def inicializar_api_client() -> HttpClient:
    """Carrega as credenciais da API e retorna um cliente HttpClient."""
...
def inicializar_session_state():
    """Garante que todas as chaves necessárias existam na sessão."""
    defaults = {
        SK.LOGGED_IN: False,
        SK.USERNAME: "",
        SK.LAST_UPDATE: None,
        SK.SIMILARITY_CACHE: None,
        SK.OPEN_COMPARISONS: set(),
        SK.PAGE_NUMBER: 0,
        SK.FULL_TEXT_DATA: None,
        SK.SHOW_FULL_TEXT_DIALOG: False,
        SK.STEP_CANCEL: None,
        SK.PROCESS_CANCEL: False,
        SK.STEP_CANCEL_PROCESSADO: False
    }
...
def as_sp(ts: pd.Timestamp | None) -> datetime | None:
    """Converte um timestamp para o fuso horário de São Paulo (America/Sao_Paulo)."""
    if pd.isna(ts): return None
    if ts.tzinfo is None: ts = ts.tz_localize(TZ_UTC)
    return ts.tz_convert(TZ_SP)

def norm(text: str | None) -> str:
    """Normaliza o texto: minúsculas, remove acentos e caracteres especiais."""
    if not isinstance(text, str): return ""
    text_sem_acento = unidecode(text.lower())
    text_limpo = re.sub(r"[^\w\s]", " ", text_sem_acento)
    return re.sub(r"\s+", " ", text_limpo).strip()

def calc_sim(text_a: str, text_b: str) -> float:
    """Calcula a similaridade entre duas strings usando token_set_ratio."""
    norm_a, norm_b = norm(text_a), norm(text_b)
...
def cor_sim(ratio: float) -> str:
    """Retorna uma cor hexadecimal baseada na taxa de similaridade."""
    if ratio >= 0.9: return "#FF5252"  # Vermelho
    if ratio >= 0.7: return "#FFB74D"  # Laranja
...
def highlight_diffs(text1: str, text2: str) -> tuple[str, str]:
    ...
    return (
        f"<pre class='highlighted-text'>{''.join(out1)}</pre>",
        f"<pre class='highlighted-text'>{''.join(out2)}</pre>"
    )
...
def db_engine() -> Engine:
    ...
    if not all([host, user, pw, db]):
        st.error("Credenciais de banco de dados ausentes.")
        st.stop()
    try:
...
def carregar_dados(eng: Engine) -> pd.DataFrame:
    """Carrega dados de atividades abertas e do histórico recente."""
    ...
    try:
        with eng.connect() as conn:
            df_abertas = pd.read_sql(query_abertas, conn)
            df_historico = pd.read_sql(query_historico, conn, params={"limite": limite_historico})
...
def gerar_mapa_similaridade(df_exibir: pd.DataFrame, df_comparar: pd.DataFrame, min_sim: float) -> tuple[dict, set]:
    """Compara cada item em df_exibir com todos os itens em df_comparar."""
    if df_exibir.empty:
        return {}, set()
...
def renderizar_sidebar(df_completo: pd.DataFrame) -> dict:
    """Renderiza a sidebar e retorna um dicionário com os filtros selecionados."""
...
def filtrar_dados(df_completo: pd.DataFrame, filtros: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Aplica os filtros e retorna os dataframes de exibição e de comparação."""
    # Base de Comparação: todo o histórico das pastas selecionadas.
    df_base_comparacao = df_completo if not filtros["pastas"] else df_completo[df_completo["activity_folder"].isin(filtros["pastas"])]

    # Itens para Exibir: aplica todos os filtros da tela.
...
def exibir_dialogo_texto_completo():
    """Mostra um diálogo com o texto completo de uma atividade."""
...
def confirmar_cancelamento(api_client: HttpClient):
    """Lida com a lógica e UI do diálogo de confirmação de cancelamento."""
    ...
    if st.session_state.get(SK.STEP_CANCEL_PROCESSADO):
        if st.button("✅ Concluir"):
            # Limpa o estado e atualiza os dados
...
def app():
    ...
    with c2:
        if st.button("Cancelar Selecionado(s)", key="process_cancel_button"):
            st.session_state[SK.STEP_CANCEL] = "confirmar"
            st.session_state[SK.STEP_CANCEL_PROCESSADO] = False
    ...
    # --- Loop de Exibição das Pastas e Atividades ---
...
def renderizar_cartao_atividade(row, pasta, sim_map, idx_map, max_selecoes, num_selecionados):
    """Renderiza um único cartão de atividade com seus detalhes e duplicatas."""
    ...
    # --- Renderização da Comparação (se houver) ---
...
def renderizar_visualizacao_comparacao(base_data_row, comp_data_dict, pasta, max_sel, num_sel):
    """Renderiza a UI de comparação lado a lado para duas atividades."""
    if not comp_data_dict: return
    
    base_id = base_data_row.activity_id
    comp_id = comp_data_dict["activity_id"]
