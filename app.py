...
import io
import difflib
from difflib import HtmlDiff   #  ➜ novo import para gerar diff com CSS
...
# CSS global – aumenta largura dos dialogs para 90 % da viewport
st.markdown("""
<style>
...
# ------------------------------------------------------------------ #
#  👇  NOVAS FUNÇÕES  para gerar e cachear o diff em HTML            #
# ------------------------------------------------------------------ #
def _gerar_diff_html(texto_a: str, texto_b: str,
                     desc_a: str = "Texto base",
                     desc_b: str = "Texto comparar") -> str:
    """
    Usa HtmlDiff.make_file() que já inclui o CSS interno
    e permite recorte de contexto para performance.
    """
    ...
    return differ.make_file(
        ...
        fromdesc=desc_a,
        todesc=desc_b,
        context=True,    # mostra apenas trechos alterados
        numlines=2       # duas linhas de contexto
    )
...
def diff_cache(id_a: int, id_b: int, txt_a: str, txt_b: str) -> str:
    """
    Cacheia o resultado da comparação para o par (id_a, id_b).
    Evita recomputar se o mesmo par for aberto novamente.
    """
    # Garante ordem estável na chave do cache
    if id_a > id_b:
...
def carregar_dados_ajustados(_engine: Engine) -> tuple[pd.DataFrame | None, Exception | None]:
    ...
    try:
        with _engine.connect() as connection:
            ...
            df_historico = pd.read_sql(query_historico, connection,
                                       params={"data_limite": data_limite_historico})
        df_combinado = pd.concat([df_abertas, df_historico], ignore_index=True)
        df_combinado.sort_values(by=['activity_id', 'activity_status'],
                                 ascending=[True, True], inplace=True)
        df_final = df_combinado.drop_duplicates(subset=['activity_id'], keep='first')
        df_final.sort_values(by=['activity_folder', 'activity_date', 'activity_id'],
...
def mostrar_texto_completo_dialog():
    ...
    if atividade_data:
        ...
        st.markdown(
            f"**Pasta:** {atividade_data['activity_folder']} | **Data:** "
            f"{atividade_data['activity_date'].strftime('%d/%m/%Y %H:%M')} | **Usuário:** "
            f"{atividade_data['user_profile_name']} | **Status:** {atividade_data['activity_status']}"
        )
        st.text_area("Texto:",
                     value=str(atividade_data['Texto']),
                     height=400, disabled=True,
                     key=f"dialog_txt_content{SUFFIX_DIALOG}_{atividade_data['activity_id']}")
...
def mostrar_comparacao_html_diff_dialog():
    ...
    st.markdown(
        f"### Comparando ID `{base_comp['activity_id']}` "
        f"com ID `{comparar_comp['activity_id']}`"
    )
    ...
    with st.spinner("Gerando comparativo…"):
        html_comparison = diff_cache(
            base_comp['activity_id'],
            comparar_comp['activity_id'],
            texto_base_comp,
            texto_comparar_comp
        )

    st.components.v1.html(html_comparison, height=600, width=1200, scrolling=True)
...
def on_click_comparar_textos_html_dialog(atividade_base, atividade_comparar):
    st.session_state[f'atividades_para_comparacao{SUFFIX_DIALOG}'] = {
        'base': atividade_base,
...
def app_principal():
    ...
    if st.sidebar.button("🔄 Atualizar Dados Base",
                         help="Busca os dados mais recentes.",
                         key=f"buscar_btn_main{SUFFIX_DIALOG}"):
    ...
    data_inicio_selecionada = st.sidebar.date_input(
        "Data de Início", value=data_inicio_padrao, key=f"di_main{SUFFIX_DIALOG}"
    )
    data_fim_selecionada = st.sidebar.date_input(
        "Data de Fim", value=data_fim_padrao, key=f"df_main{SUFFIX_DIALOG}"
    )
    ...
    if df_atividades_periodo.empty:
        st.info(f"Nenhuma atividade para o período de "
                f"{data_inicio_selecionada.strftime('%d/%m/%Y')} a "
                f"{data_fim_selecionada.strftime('%d/%m/%Y')}.")
    else:
        st.success(f"**{len(df_atividades_periodo)}** atividades no período "
                   f"({len(df_raw_total)} total carregado).")

    st.sidebar.markdown("---")
    st.sidebar.subheader("2. Filtros de Análise")
    pastas_disp = sorted(df_atividades_periodo['activity_folder'].dropna().unique()) \
        if not df_atividades_periodo.empty else []
    pastas_sel = st.sidebar.multiselect(
        "Analisar Pasta(s):", pastas_disp, default=[], key=f"pasta_sel{SUFFIX_DIALOG}_main"
    )
    status_disp_analise = sorted(df_atividades_periodo['activity_status'].dropna().unique()) \
        if not df_atividades_periodo.empty else []
    status_sel_analise = st.sidebar.multiselect(
        "Analisar Status:", status_disp_analise, default=[], key=f"status_sel{SUFFIX_DIALOG}_main"
    )
    ...
    min_sim = st.sidebar.slider(
        "Similaridade ≥ que (%):", 0, 100, 70, 5,
        key=f"sim_slider{SUFFIX_DIALOG}_main"
    ) / 100.0
    apenas_dup = st.sidebar.checkbox(
        "Exibir apenas com duplicatas", value=True, key=f"dup_cb{SUFFIX_DIALOG}_main"
    )
    pastas_multi_user = {
    ...
    apenas_multi = st.sidebar.checkbox(
        "Exibir pastas com múltiplos usuários", False, key=f"multi_cb{SUFFIX_DIALOG}_main"
    )
    usuarios_disp_ex = sorted(df_para_analise['user_profile_name'].dropna().unique()) \
        if not df_para_analise.empty else []
    usuarios_sel = st.sidebar.multiselect(
        "Exibir Usuário(s):", usuarios_disp_ex, default=[], key=f"user_sel{SUFFIX_DIALOG}_main"
    )

    # ---------------------------------------------------------------------------------
    #  Cálculo de similaridade (o código original é mantido – não houve mudança aqui)
    # ---------------------------------------------------------------------------------
    ...
    if not df_para_analise.empty and len(df_para_analise) > 1:
        ...
        total_pastas = df_para_analise['activity_folder'].nunique()
        ...
        for nome_pasta_calc, df_pasta_calc in df_para_analise.groupby('activity_folder'):
            ...
            if len(df_pasta_calc) < 2:
                ...
                if total_pastas > 0:
                    prog_bar.progress(pastas_processadas / total_pastas)
                continue
        ...
        for act_id_sort in map_id_para_similaridades:
            map_id_para_similaridades[act_id_sort].sort(
                key=lambda x: x['ratio'], reverse=True
            )

    df_exibir = df_para_analise.copy()
    if apenas_dup:
        df_exibir = df_exibir[df_exibir['activity_id'].isin(atividades_com_duplicatas_calculadas)]
    if apenas_multi:
    ...
    # --------------------------------------------------------------------------
    # Exportação XLSX (sem mudança relevante no código original)
    # --------------------------------------------------------------------------
    st.sidebar.markdown("---")
        ...
        if df_exibir.empty:
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                ...
                for id_base_export, lista_similares_export in map_id_para_similaridades.items():
                    if id_base_export not in df_exibir['activity_id'].values:
                        continue
                    for sim_info_export in lista_similares_export:
                        detalhes_similar_export_rows = df_atividades_periodo[
                            df_atividades_periodo['activity_id'] == sim_info_export['id_similar']
                        ]
                        if detalhes_similar_export_rows.empty:
                            continue
            ...
            st.sidebar.download_button(
                "Baixar XLSX", output.getvalue(),
                f"duplicatas_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )

    # --------------------------------------------------------------------------
    # Parte visual – iterando pastas e atividades (sem mudança estrutural)
    # --------------------------------------------------------------------------
    ...
    for nome_pasta in pastas_ordenadas:
        ...
        with st.expander(titulo, expanded=False):
            for _, atividade_row in df_pasta_exibicao.iterrows():
                ...
                with col_info:
                    st.markdown(
                        f"**ID:** `{atividade['activity_id']}` | **Data:** "
                        f"{atividade['activity_date'].strftime('%d/%m/%Y %H:%M')} | "
                        f"**Status:** `{atividade['activity_status']}`"
                    )
                    st.markdown(f"**Usuário:** {atividade['user_profile_name']}")
                    st.text_area("Texto:", str(atividade['Texto']), height=100,
                                 key=f"texto_exp{SUFFIX_DIALOG}_{nome_pasta}_{atividade['activity_id']}",
                                 disabled=True)
                    btn_cols = st.columns(3)
                    links = gerar_links_zflow(atividade['activity_id'])
                    if btn_cols[0].button(
                        "👁️ Ver Completo",
                        key=f"ver_completo_btn{SUFFIX_DIALOG}_{atividade['activity_id']}",
                        on_click=on_click_ver_texto_completo, args=(atividade,)
                    ):
                        pass
                ...
                with col_sim_display:
                    ...
                    if similares_para_esta_atividade:
                        st.markdown(
                            f"**<span style='color:red;'>Duplicatas (Intra-Pasta):</span>** "
                            f"({len(similares_para_esta_atividade)})",
                            unsafe_allow_html=True
                        )
                        for sim_data in similares_para_esta_atividade:
                            info_dupe_rows = df_atividades_periodo[
                                df_atividades_periodo['activity_id'] == sim_data['id_similar']
                            ]
                            if info_dupe_rows.empty:
                                st.caption(
                                    f"Detalhes da ID {sim_data['id_similar']} não disponíveis "
                                    "(fora do período de datas ou filtros de análise)."
                                )
                                continue
                            info_dupe = info_dupe_rows.iloc[0].to_dict()
                            container_dup = st.container(border=True)
                            container_dup.markdown(
                                f"""<small><div style='background-color:{sim_data['cor']};
                                       padding:3px 6px;border-radius:5px;color:black;margin-bottom:5px;
                                       font-weight:500;'>
                                    <b>ID: {info_dupe['activity_id']}
                                    ({sim_data['ratio']:.0%})</b><br>
                                    Data: {info_dupe['activity_date'].strftime('%d/%m/%y %H:%M')}
                                    | Status: {info_dupe['activity_status']}<br>
                                    Usuário: {info_dupe['user_profile_name']}
                                    </div></small>""",
                                unsafe_allow_html=True
                            )
                            if container_dup.button(
                                "⚖️ Comparar (Detalhado)",
                                key=f"comp_html_dialog_btn{SUFFIX_DIALOG}_"
                                    f"{atividade['activity_id']}_{info_dupe['activity_id']}",
                                on_click=on_click_comparar_textos_html_dialog,
                        ...
                        if not apenas_dup:
                            st.markdown(
                                "**<span style='color:green;'>Sem duplicatas "
                                "(nesta análise)</span>**", unsafe_allow_html=True
                            )

    # -------------------------------------------------------
    # Exibe dialogs, se estiverem ativados no session_state
    # -------------------------------------------------------
...
def check_credentials(username, password):
    try:
        ...
        if username in user_creds and str(user_creds[username]) == password:
            return True
    except KeyError:
        return False
    except Exception:
...
def login_form():
    ...
    with st.form(f"login_form{SUFFIX_DIALOG}_main"):
        ...
        password = st.text_input("Senha", key=f"login_password{SUFFIX_DIALOG}_main",
                                 type="password")
